	...
	BEGIN TRANSACTION;
		...
		;WITH JourneyEvent AS (
			SELECT
				...
				,Journey_ID    
				,JourneyEvent_RawID
				-- To identify true journey start/end if EventTimeStamp is duplicated:
				,EventOrder =
					CASE EventTypeID
						WHEN 1 THEN 1
						WHEN 2 THEN 3
				...
				,DistanceFromPrevPosition =
					CASE
						WHEN DistanceFromPrevPosition < 0    THEN 0
						WHEN DistanceFromPrevPosition > 2000 THEN 2000
															 ELSE DistanceFromPrevPosition
					END
				-- Cap anomalous data similar to RT Score:
				,TravelSpeedMPH =
					CASE
						WHEN TravelSpeedMPH < 0   THEN 0
						WHEN TravelSpeedMPH > 156 THEN 156
												  ELSE TravelSpeedMPH
					END
			...
			WHERE NOT EXISTS (
				...
				WHERE je.IMEI = rp.IMEI
					AND je.Journey_ID = rp.Journey_ID
			)
				-- Because there are nullability constraints on these fields:
				AND IMEI 	   IS NOT NULL
				AND Journey_ID IS NOT NULL
				-- Because the Point function will error with a NULL input:
				AND Latitude   IS NOT NULL
				AND Longitude  IS NOT NULL
		)
		-- The TABLOCK table hint is used to reduce logging:
		...
		;WITH RowNumbers AS (
			...
			FROM staging.dbo.PolygonRTJE2 je
			LEFT JOIN #BST bst
				ON  je.EventTimeStamp >= bst.StartTime
				AND je.EventTimeStamp <  bst.EndTime
		)
		INSERT staging.dbo.PolygonRTJE3 WITH (TABLOCK) (
		...
		INSERT DataSummary.dbo.RedTailPolygons (
		...
		SELECT
			...
			,TimePctOfJourney
			,EventsPctOfJourney
		FROM staging.dbo.PolygonRTJE3
		;

		INSERT @Log (ScriptStartTime, Query, RowsAffected, QueryFinishTime)
		SELECT
			 @ScriptStartTime
			,'Insert into DataSummary.dbo.RedTailPolygons'
			,@@ROWCOUNT
			,SYSDATETIME()
		;
		...
		IF OBJECT_ID(N'sandbox.dbo.RedTailPolygons_Log', N'U') IS NULL
		BEGIN
			SELECT
				 ScriptStartTime
				,Query
				,RowsAffected
				,QueryFinishTime
			INTO sandbox.dbo.RedTailPolygons_Log
			FROM @Log
			;
		END
		ELSE
		BEGIN
			INSERT sandbox.dbo.RedTailPolygons_Log (ScriptStartTime, Query, RowsAffected, QueryFinishTime)
			SELECT
				 ScriptStartTime
				,Query
				,RowsAffected
